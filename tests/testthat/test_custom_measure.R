...
test_that("Can create custom measure", {
  ...
  m5 = msr("multiout.custom",
    measures = list(classif = cl, regr = rg),
    weights = c(classif = 1, regr = 0)
  )
  m6 = msr("multiout.custom",
    measures = list(X2d = cl, wav = msr("classif.fp"), t3 = rg),
    weights = c(classif = 1, regr = 0)
  )
  ...
  expect_number(p$score(m3), lower = 0, upper = 1)
  s4 = p$score(m4)
  ...
  expect_number(p$score(m6), lower = 0, upper = 1)
})